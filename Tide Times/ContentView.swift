...
//  Tide Times
//
//  Created by Tatiana Kornilova on 15.11.2024.
...
class TideViewModel: ObservableObject {
    ...
    @Published var predefinedLocations = [
        "San Francisco, CA",
        "Seattle, WA",
        "Miami, FL",
        "Boston, MA",
  ...
  //  private let apiKey = "97505ca5-7a08-4d70-8cb6-9cd003cb08b0" // Get free API key from https://www.worldtides.info/developer
    private let apiKey = "f5c1814e-a307-465e-8de4-55b42cdc9aa3" // Get free API key from https://www.worldtides.info/developer
    ...
    func searchLocations(query: String) {
        guard !query.isEmpty else {
            suggestedLocations = predefinedLocations
            return
        }
        
        // Filter predefined locations first
        let filteredPredefined = predefinedLocations.filter { $0.localizedCaseInsensitiveContains(query) }
        
        // Then do geocoding search
        let geocoder = CLGeocoder()
        geocoder.geocodeAddressString(query) { [weak self] places, error in
            DispatchQueue.main.async {
                let geocodedLocations = places?.compactMap { $0.name } ?? []
                // Combine predefined filtered locations with geocoded ones
                self?.suggestedLocations = Array(Set(filteredPredefined + geocodedLocations))
            }
        }
    }
